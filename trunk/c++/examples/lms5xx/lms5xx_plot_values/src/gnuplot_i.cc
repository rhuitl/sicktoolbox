...
void Gnuplot::plot_xy(vector<double> x, vector<double> y, const string &title)
{
    ...
    /*if (this->nplots > 0)
        ...
        cmdstr << "plot ";
    if (title == "")
        cmdstr << "'-' with " << this->pstyle << endl;
    else
        cmdstr << "'-' title \"" << title << "\" with " << this->pstyle << endl;

    //
    // write the data to command
    //
    for (vector<double>::size_type i = 0; i < x.size(); i++)
        cmdstr << x[i] << " " << y[i] << endl;
    cmdstr << "e" << endl;
...
void Gnuplot::plot_xy2(vector<double> x, vector<double> y, vector<double> x2, vector<double> y2, const string &title)
{
    ostringstream cmdstr;
    ...
    for (vector<double>::size_type i = 0; i < x.size(); i++)
        cmdstr << x[i] << " " << y[i] << endl;
    cmdstr << "e" << endl;
    for (vector<double>::size_type i = 0; i < x2.size(); i++)
        cmdstr << x2[i] << " " << y2[i] << endl;
    cmdstr << "e" << endl;
...
void Gnuplot::plot_xy5(vector<double> x, vector<double> y,
                       ...
                       const string &title)
{
    ...
    if (title == "")
        cmdstr << "'-' with " << this->pstyle << ","
               ...
               << "'-' with " << this->pstyle << ","
               << "'-' with " << this->pstyle << ","
               << "'-' with " << this->pstyle << endl;
    else
        cmdstr << "'-' title \"" << title << "\" with " << this->pstyle << ","
               << "'-' title \"" << title << "\" with " << this->pstyle << ","
               << "'-' title \"" << title << "\" with " << this->pstyle << ","
               << "'-' title \"" << title << "\" with " << this->pstyle << ","
               << "'-' title \"" << title << "\" with " << this->pstyle << endl;

    //
    // write the data to command
    //
    cmdstr << "0 0" << endl;                     // dummy point if no other points are present
    for (vector<double>::size_type i = 0; i < x.size(); i++)
        cmdstr << x[i] << " " << y[i] << endl;
    cmdstr << "e" << endl;
    ...
    cmdstr << "0 0" << endl;                     // dummy point if no other points are present
    for (vector<double>::size_type i = 0; i < x3.size(); i++)
        cmdstr << x3[i] << " " << y3[i] << endl;
    cmdstr << "e" << endl;

    cmdstr << "0 0" << endl;                     // dummy point if no other points are present
    for (vector<double>::size_type i = 0; i < x4.size(); i++)
        cmdstr << x4[i] << " " << y4[i] << endl;
    cmdstr << "e" << endl;

    cmdstr << "0 0" << endl;                     // dummy point if no other points are present
    for (vector<double>::size_type i = 0; i < x5.size(); i++)
        cmdstr << x5[i] << " " << y5[i] << endl;
    cmdstr << "e" << endl;
